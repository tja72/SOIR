...
public:
	...
	bool LoadImageFromRAWFile(std::string filename) {
		...
		if (m_colorImageHeight != header.height || m_colorImageWidth != header.width || m_colorImagesTimeStamps[m_currentIdx] != header.timestamp || IMAGE != header.filetype) {
	...
	bool LoadDepthFromRAWFile(std::string filename) {
		...
		// check width, height, timestamp and format
		if (m_depthImageHeight != header.height || m_depthImageWidth != header.width || m_depthImagesTimeStamps[m_currentIdx] != header.timestamp || DEPTH != header.filetype) {
			std::cerr << "Header error while trying to read file " << filename << "\n" << std::endl;
			return false;
		}
