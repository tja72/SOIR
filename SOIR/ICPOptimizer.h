...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		//residual = diff to min



		PoseIncrement<T> transf(const_cast<T*>(pose));

		T out[3] = { T(0.0), T(0.0), T(0.0) };
		T in[3] = { T(m_sourcePoint.x()), T(m_sourcePoint.y()), T(m_sourcePoint.z()) };
		transf.apply(in, out);
		residuals[0] = T(T(m_weight)*(out[0] - T(m_targetPoint.x())));
		residuals[1] = T(T(m_weight)*(out[1] - T(m_targetPoint.y())));
		residuals[2] = T(T(m_weight)*(out[2] - T(m_targetPoint.z())));
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		T in[3] = { T(m_sourcePoint.x()), T(m_sourcePoint.y()), T(m_sourcePoint.z()) };
		transf.apply(in, out);
		T diff[3] = { T(out[0] - T(m_targetPoint.x())), T(out[1] - T(m_targetPoint.y())), T(out[2] - T(m_targetPoint.z())) };
		



		residuals[0] = T(m_weight)*T(T(m_targetNormal.x()) * diff[0] + T(m_targetNormal.y()) * diff[1] + T(m_targetNormal.z()) * diff[2]);

		return true;
	}

	static ceres::CostFunction* create(const Vector3f& sourcePoint, const Vector3f& targetPoint, const Vector3f& targetNormal, const float lambda) {
		return new ceres::AutoDiffCostFunction<PointToPlaneConstraint, 1, 6>(
			new PointToPlaneConstraint(sourcePoint, targetPoint, targetNormal, lambda)
		);
	}
...
public:
	...
	Matrix4f estimatePose(const PointCloud& source, const PointCloud& target, Matrix4f initialPose = Matrix4f::Identity()) {
		...
		for (int i = 0; i < m_nIterations; ++i) {
			...
			prepareConstraints(transformedPoints, target.getPoints(), target.getNormals(), matches, poseIncrement, problem); //problem, poseIncrement
	...
	void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				ceres::CostFunction* costPoint = PointToPointConstraint::create(sourcePoint, targetPoint, .1);
				problem.AddResidualBlock(costPoint, nullptr, poseIncrement.getData());
				...
				if (m_bUsePointToPlaneConstraints) {
					...
					// to the Ceres problem.
					ceres::CostFunction* costPlane = PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, 1.);
					problem.AddResidualBlock(costPlane, nullptr, poseIncrement.getData());
