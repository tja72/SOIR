...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		T difference[3] = { T(out[0] - T(m_targetPoint.x())), T(out[1] - T(m_targetPoint.y())), T(out[2] - T(m_targetPoint.z())) };

		residuals[0] = T(m_weight)*T(T(m_targetNormal.x()) * difference[0] + T(m_targetNormal.y()) * difference[1] + T(m_targetNormal.z()) * difference[2]);
...
public:
	...
	Matrix4f estimatePose(const PointCloud& source, const PointCloud& target, Matrix4f initialPose = Matrix4f::Identity()) {
		...
		for (int i = 0; i < m_nIterations; ++i) {
			...
			prepareConstraints(transformedPoints, target.getPoints(), target.getNormals(), matches, poseIncrement, problem); // TODO target.getNormals()
			...
			std::cout << summary.BriefReport() << std::endl; // TODO: back to BriefReport
	...
	void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				ceres::CostFunction* constraint1 = PointToPointConstraint::create(sourcePoint, targetPoint, 0.1f);
				problem.AddResidualBlock(constraint1, nullptr, poseIncrement.getData());
				...
				if (m_bUsePointToPlaneConstraints) {
					...
					// to the Ceres problem.
					ceres::CostFunction* constraint2 = PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, 1.0f);
					problem.AddResidualBlock(constraint2, nullptr, poseIncrement.getData());
				}
			}
		}
	}
