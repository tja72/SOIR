...
void openCommon()
{
	...
	{
		if (g_Image.IsCapabilitySupported(XN_CAPABILITY_FRAME_SYNC))
		{
			if (g_Image.GetFrameSyncCap().CanFrameSyncWith(g_Depth))
			{
				nRetVal = g_Image.GetFrameSyncCap().FrameSyncWith(g_Depth);
				if (nRetVal != XN_STATUS_OK)											\
				{																\
					printf("Syncing Depth and Image failed: %s\n", xnGetStatusString(nRetVal));		\
				}
			}
		}
	}
	else {
		printf("Depth or Image is off");
	}**/
